defmodule CA.CRT do
  ...
  def baseLength(oid) when is_tuple(oid) do
    CA.Curve.getLength(CA.KnownCurves.getCurveByOid(oid))
  end
  ...
  # Rivest-Shamir-Adleman schema of Diffie–Hellman key exchange
  def decodeRSA(_, _, publicKey) do
  ...
  # Integers over Prime Galois Fields GF(p)
  def decodeECC(agreement, parameters, publicKey) do
  ...
  # Polynomials over Binary Galois Fields GF(2^m)
  def decodeGFE(agreement, params, publicKey) do
  ...
  def decodePublicKey(agreement, {:asn1_OPENTYPE, params}, publicKey) do
    decodePublicKey(agreement, params, publicKey)
  end

  def decodePublicKey(agreement, params, publicKey) do
    case agreement do
      {1, 2, 840, 113_549, 1, 1, 1} ->
        decodeRSA(agreement, params, publicKey)

      {1, 2, 840, 10045, 2, 1} ->
        decodeECC(agreement, params, publicKey)

      {1, 2, 804, 2, 1, 1, 1, 1, 3, 1, 1} ->
        decodeGFE(agreement, params, publicKey)

      _ ->
        :logger.info(~c"new unknown publicKey agreement scheme detected: ~p~n", [agreement])
        :base64.encode(publicKey)
    end
  end

  def parseCertPEM(file) do
    {:ok, bin} = :file.read_file(file)
    list = :public_key.pem_decode(bin)
    :lists.map(fn x -> parseCert(:public_key.pem_entry_decode(x)) end, list)
  end

  def parseCertB64(file) do
    {:ok, bin} = :file.read_file(file)
    parseCertBin97(:base64.decode(bin))
  end
  ...
  def parseCertBin97(bin) do
    {:ok, cert} = :AuthenticationFramework.decode(:Certificate, bin)
    parseCert(cert)
  end

  def parseCertBin88(bin) do
    {:ok, cert} = :PKIX1Explicit88.decode(:Certificate, bin)
    parseCert(cert)
  end

  def parseCert(cert, _) do
    parseCert(cert)
  end

  def parseCert({:certificate, cert}) do
    parseCert(cert)
  end
