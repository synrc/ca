...
-asn1_info([{vsn,'5.2.1'},
 ...
 {cwd,"/Users/maxim/depot/synrc/ca/priv/kep"},
 {outdir,"/Users/maxim/depot/synrc/ca/priv/kep"},
 {i,"."},
 {i,"/Users/maxim/depot/synrc/ca/priv/kep"}]}]).
...
encode_disp('TimeStampToken', Data) -> enc_TimeStampToken(Data);
encode_disp('TSTInfo', Data) -> enc_TSTInfo(Data);
encode_disp('Accuracy', Data) -> enc_Accuracy(Data);
...
decode_disp('TimeStampToken', Data) -> dec_TimeStampToken(Data);
decode_disp('TSTInfo', Data) -> dec_TSTInfo(Data);
decode_disp('Accuracy', Data) -> dec_Accuracy(Data);
...
%%  ContentTimeStamp
%%================================
enc_ContentTimeStamp(Val) ->
    enc_ContentTimeStamp(Val, [<<48>>]).

enc_ContentTimeStamp(Val, TagIn) ->
   enc_TimeStampToken(Val, TagIn).


dec_ContentTimeStamp(Tlv) ->
   dec_ContentTimeStamp(Tlv, [16]).

dec_ContentTimeStamp(Tlv, TagIn) ->
'dec_TimeStampToken'(Tlv, TagIn).
...
%%  TimeStampResp
%%================================
enc_TimeStampResp(Val) ->
    enc_TimeStampResp(Val, [<<48>>]).

enc_TimeStampResp(Val, TagIn) ->
{_,Cindex1,Cindex2} = Val,

%%-------------------------------------------------
%% attribute status(1)   External PKIXCMP-2009:PKIStatusInfo
%%-------------------------------------------------
   {EncBytes1,EncLen1} = 'PKIXCMP-2009':'enc_PKIStatusInfo'(Cindex1, [<<48>>]),

%%-------------------------------------------------
%% attribute timeStampToken(2)   External KEP:TimeStampToken OPTIONAL
%%-------------------------------------------------
   {EncBytes2,EncLen2} =  case Cindex2 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            'enc_TimeStampToken'(Cindex2, [<<48>>])
       end,
...
dec_TimeStampResp(Tlv) ->
   dec_TimeStampResp(Tlv, [16]).

dec_TimeStampResp(Tlv, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
Tlv1 = match_tags(Tlv, TagIn),
...
%%-------------------------------------------------
[V1|Tlv2] = Tlv1, 
Term1 = 'PKIXCMP-2009':'dec_PKIStatusInfo'(V1, [16]),
...
{Term2,Tlv3} = case Tlv2 of
[{16,V2}|TempTlv3] ->
    {'dec_TimeStampToken'(V2, []), TempTlv3};
    _ ->
        { asn1_NOVALUE, Tlv2}
end,
...
Res1 = {'TimeStampResp',Term1,Term2},
Res1.
...
%%  TimeStampToken
%%================================
enc_TimeStampToken(Val) ->
    enc_TimeStampToken(Val, [<<48>>]).

enc_TimeStampToken(Val, TagIn) ->
   enc_ContentInfo(Val, TagIn).


dec_TimeStampToken(Tlv) ->
   dec_TimeStampToken(Tlv, [16]).
...
%%-------------------------------------------------
%% attribute policy(2) with type OBJECT IDENTIFIER
%%-------------------------------------------------
   {EncBytes2,EncLen2} = encode_object_identifier(Cindex2, [<<6>>]),

%%-------------------------------------------------
%% attribute messageImprint(3)   External KEP:MessageImprint
%%-------------------------------------------------
   {EncBytes3,EncLen3} = 'enc_MessageImprint'(Cindex3, [<<48>>]),
...
%%-------------------------------------------------
%% attribute genTime(5) with type GeneralizedTime
%%-------------------------------------------------
   {EncBytes5,EncLen5} = encode_restricted_string(Cindex5, [<<24>>]),

%%-------------------------------------------------
%% attribute accuracy(6)   External KEP:Accuracy OPTIONAL
%%-------------------------------------------------
   {EncBytes6,EncLen6} =  case Cindex6 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            'enc_Accuracy'(Cindex6, [<<48>>])
       end,

%%-------------------------------------------------
%% attribute nonce(7) with type INTEGER OPTIONAL
%%-------------------------------------------------
   {EncBytes7,EncLen7} =  case Cindex7 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            encode_integer(Cindex7, [<<2>>])
       end,

%%-------------------------------------------------
%% attribute tsa(8)   External KEP:GeneralName OPTIONAL
%%-------------------------------------------------
   {EncBytes8,EncLen8} =  case Cindex8 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            'enc_GeneralName'(Cindex8, [<<160>>])
       end,
...
%%-------------------------------------------------
   {EncBytes9,EncLen9} =  case Cindex9 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            'AuthenticationFramework':'enc_Extensions'(Cindex9, [<<161>>])
       end,
...
%%-------------------------------------------------
[V3|Tlv4] = Tlv3, 
Term3 = 'dec_MessageImprint'(V3, [16]),
...
[V5|Tlv6] = Tlv5, 
Term5 = begin
binary_to_list(decode_restricted_string(V5, [24]))
end
,
...
[{131073,V9}|TempTlv10] ->
    {'AuthenticationFramework':'dec_Extensions'(V9, []), TempTlv10};
    _ ->
        { asn1_NOVALUE, Tlv9}
end,
...
Res1 = {'TSTInfo',Term1,Term2,Term3,Term4,Term5,Term6,Term7,Term8,Term9},
Res1.
...
enc_Accuracy(Val, TagIn) ->
{_,Cindex1,Cindex2,Cindex3} = Val,

%%-------------------------------------------------
%% attribute seconds(1) with type INTEGER OPTIONAL
%%-------------------------------------------------
   {EncBytes1,EncLen1} =  case Cindex1 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
...
%%-------------------------------------------------
%% attribute millis(2) with type INTEGER OPTIONAL
%%-------------------------------------------------
   {EncBytes2,EncLen2} =  case Cindex2 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
...
%%-------------------------------------------------
%% attribute micros(3) with type INTEGER OPTIONAL
%%-------------------------------------------------
   {EncBytes3,EncLen3} =  case Cindex3 of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
...
Res1 = {'Accuracy',Term1,Term2,Term3},
Res1.
